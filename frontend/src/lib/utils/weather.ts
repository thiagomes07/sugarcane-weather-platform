...
export function processWeatherData(
  ...
  const currentProcessed = {
    ...
    conditionDescription: translateWeatherDescription(current.weather[0].description),
 ...
 */
function groupForecastByDay(forecast: ForecastItem[]): ProcessedWeatherData['forecast'] {
 ...
 */
export function getBestWorkingHours(sunrise: Date, sunset: Date): {
  morning: string;
  afternoon: string;
} {
